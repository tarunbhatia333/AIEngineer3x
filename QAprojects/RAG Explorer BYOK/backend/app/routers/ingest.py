...
async def reingest_default(keys: ApiKeys = Depends(get_api_keys)):
    try:
        count = await ingest.ingest_default(openai_key=keys.openai, pinecone_key=keys.pinecone, force=True)
...
async def upload_and_ingest(file: UploadFile = File(...), keys: ApiKeys = Depends(get_api_keys)):
    allowed = (".pdf", ".txt", ".md")
